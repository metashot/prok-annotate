...
parser.add_argument('-s', '--suffix', dest='suffix', required=False,
                    help='filename suffix', default='._kofam_hits.txt')              
...
for input_fn in args.input:
    ...
    input_df = input_df.groupby(["ko", "modules_with_ko", "ko_definition"]) \
        [["unique_id"]].nunique()
    input_df = input_df.rename(columns = {"unique_id": input_id})
    compl_df = pd.concat([compl_df, input_df], axis=1, join='outer')

compl_df.fillna(0, inplace=True)
compl_df.to_csv('anvio_kofam_hits.tsv', sep='\t', float_format='%.d')
